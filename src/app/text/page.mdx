...
## x, y, dominant-baseline

By default the **x** attribute positions the text element horizontally using it's left edge. The **y** attribute is used to position the text relative to its **dominant-baseline**, which is, by default, directly under the text. In the image below the dominant baseline is represented by the black line.
...
While dominant-baseline is used to set the vertical alignment, the **text-anchor** attribute is used to set the **horizontal** alignment of the text element. Each of the three text elements below has the same x position (250, the middle of the svg), but different text-anchor positions.
