...
Before we learn about the shape elements, let's create a basic SVG in our <a href={"https://codepen.io/aokorodu/pen/bGZWyaN"} target="\_blank">{"starter codepen practice page"}</a>. We'll start with the **\<svg\>** element, which is the parent container for all our shape elements.
...
There are other attributes that we can add to our svg element, like **viewport** and **namespace**, but this is all we need for now. We'll learn about some advanced svg attributes in our <a href={"/syntax"} target="\_blank">{"syntax"}</a> chapter.
