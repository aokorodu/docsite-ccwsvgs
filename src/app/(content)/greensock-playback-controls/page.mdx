...
You'll notice that the starter page contains four buttons below the SVG - **play**, **pause**, **reverse** and **restart**. We'll be connecting these buttons to the methods we'll use to control our animation. I've created variables for all the UI elements:
...
let theTween = null;
let tweenDuration = 0;`}</Blocks>

We can set the value of tweenDuration using the **duration()** method once we create our timeline.
...
<Blocks>{`function animate() {
  ...
  tweenDuration = theTween.duration();
}`}</Blocks>
